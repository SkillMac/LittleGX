...
public class Cup : MonoBehaviour {
    ...
    public GameObject[] AllGam;

    public GameObject WinLine;

    //杯子内容器的四个顶点坐标
    private Vector3[] AllVers;
    //赢得最低分数线
    private Vector3 LineWin;

    private void Awake()
    {
        Instance = this;

        AllVers = new Vector3[AllGam.Length];

        for(int i =0;i< AllGam.Length;i++)
        {
            AllVers[i] = AllGam[i].transform.position;
        }

        LineWin = WinLine.transform.position;
    }
